...
util.nav = function (url) {
	wx.navigateTo({
		url: url,
	})
};
util.notice = function (str) {
	wx.showToast({
		title: str,
		icon: 'success',
		duration: 1000
	...
	var promise = new Promise((resolve, reject) => {
		wx.getLocation({
			...
			success: function (res) {
				wx.setStorageSync('position', res.latitude.toString() + ',' + res.longitude.toString());
...
util.get = function (url, data = {}) {
	var app = getApp();
	var promise = new Promise((resolve, reject) => {
		let getUrl = util.url(url);
		getUrl = getUrl + '&m=fy_jiu' + '&uniacid=' + app.siteInfo.uniacid;
		if (!data.openid) {
			getUrl = getUrl + '&openid=' + wx.getStorageSync('openid')
		}
		var that = this;
		var postData = data;
		wx.request({
...
//处理自定义底部菜单
util.handleTabbar = function (currentTab) {
	
	util.get('entry/wxapp/TabBar', {}).then(res => {
		...
		let bottomHeight;
		isPhone ? bottomHeight = '138rpx' : bottomHeight = '80rpx';
		blist['bottomHeight'] = bottomHeight;
		currentPage.setData({
			blist: blist,
			tcolor: blist.tcolor
		})
		// wx.setNavigationBarColor({
		//   frontColor: blist.font_color,
		//   backgroundColor: blist.head_color,
		// })
		if (currentTab == 'index') {
			wx.setNavigationBarColor({
				frontColor: '#000000',
		...
		wx.setStorageSync('blist', blist)
	})
};
//处理商户页面的自定义底部菜单
util.handleTabbar2 = function (currentTab) {
	util.get('entry/wxapp/TabBar', {}).then(res => {
		console.log('handleTabbar');
		var pages = getCurrentPages()
		var currentPage = pages[pages.length - 1]
		var app = getApp();
		var blist = res.data;
		...
		let isPhone = app.globalData.isIphoneX;
		let bottomHeight;
		isPhone ? bottomHeight = '138rpx' : bottomHeight = '112rpx';
		blist['bottomHeight'] = bottomHeight;
		currentPage.setData({
			blist: blist,
			tcolor: blist.tcolor
...
// 处理版权跳转小程序和拨打电话
util.handelCopyAction = function () {
	let blist = wx.getStorageSync('blist');
...
//处理图片上传
util.upload = function (uploadUrl, data, filename,name) {
	var app = getApp();
